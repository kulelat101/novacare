...
export const MODULES = [
  ...
  { label: 'Select Patient', href: '/patients/select', group: 'Admission', icon: 'UserCircleIcon', roles: [ROLES.DOCTOR, ROLES.NURSE] },
  ...
  { label: 'Laboratory', href: '/diagnostics/laboratory', group: 'Diagnostics', icon: 'BuildingLibraryIcon', roles: [ROLES.PATIENT, ROLES.DOCTOR, ROLES.NURSE] },
  { label: 'Imaging', href: '/diagnostics/imaging', group: 'Diagnostics', icon: 'PhotoIcon', roles: [ROLES.PATIENT, ROLES.DOCTOR, ROLES.NURSE] },
  { label: 'Billing', href: '/discharge-and-billing/billing', group: 'Discharge & Billing', icon: 'CreditCardIcon', roles: [ROLES.PATIENT, ROLES.DOCTOR, ROLES.NURSE] },
  { label: 'Discharge', href: '/discharge-and-billing/discharge', group: 'Discharge & Billing', icon: 'ArrowRightOnRectangleIcon', roles: [ROLES.PATIENT, ROLES.DOCTOR, ROLES.NURSE] },
  { label: 'Login Logs', href: '/admin/login-logs', group: 'Admin', icon: 'ClockIcon', roles: [ROLES.DOCTOR, ROLES.NURSE, ROLES.STAFF, ROLES.ADMIN] },
];

export function canAccess(role, href) {
  const normalizedRole = String(role || '').trim().toLowerCase();
  if (href === '/change-password') return Boolean(normalizedRole);
...
export function getDefaultRoute(role) {
  const normalizedRole = String(role || '').trim().toLowerCase();
  if (normalizedRole === ROLES.PATIENT) return '/discharge-and-billing/billing';
