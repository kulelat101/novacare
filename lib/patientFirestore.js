...
async function loadRowsFromCollectionGroup(collectionName, candidateRecordIds) {
  const rows = [];
  const seen = new Set();
  let permissionDeniedCount = 0;
  let lastNonPermissionError = null;
  ...
  for (const fieldName of fieldsToSearch) {
    for (const candidateId of candidateRecordIds) {
      try {
        const snapshot = await getDocs(
          query(
            collectionGroup(db, collectionName),
            where(fieldName, '==', candidateId)
          )
        );
        ...
        snapshot.docs.forEach((item) => {
          ...
          const patientDocumentId = pathParts[0] === 'patients' ? pathParts[1] : '';
          rows.push(mapRecordSnapshotDoc(item, patientDocumentId));
        });
      } catch (err) {
        if (isPermissionError(err)) {
...
export async function loadPatientRows(
  ...
  let rows = directResult.rows;

  // Fallback for older records that were saved under a different patient document
  // path but still contain patientId / visiblePatientId fields. This is what
  // lets patient accounts see historical billing, discharge, and laboratory
  // records created before the canonical Firestore document ID fix.
  ...
  const permissionDeniedCount = directResult.permissionDeniedCount + collectionGroupResult.permissionDeniedCount;
  const lastNonPermissionError = directResult.lastNonPermissionError || collectionGroupResult.lastNonPermissionError;

  if (rows.length === 0 && lastNonPermissionError) {
    throw lastNonPermissionError;
  }

  if (rows.length === 0 && permissionDeniedCount > 0) {
    const err = new Error(
      'Firestore rules blocked access to this patient record collection. Allow linked patient accounts to read their own billing, discharge, laboratory, and imaging records.'
    );
